...
public class SettingFragment extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        b.setOnClickListener(view -> {
            ...
            if(!s.isEmpty())
            {
                DatabaseReference db=FirebaseDatabase.getInstance().getReference("Student/"+uid+"/sname");
                db.setValue(s);
            ...
            if(!ph.isEmpty())
            {
                DatabaseReference db=FirebaseDatabase.getInstance().getReference("Student/"+uid+"/mn");
                db.setValue(ph);
                System.out.println("here");
                phone.setText("");
        ...
        d.setOnClickListener(view -> {
            ...
            user1.delete().addOnCompleteListener(task -> {
                if(task.isSuccessful())
                {
                    ...
                    ref.child(uid1).removeValue();

                    Toast.makeText(getApplicationContext(),"deleted "+ user1.getEmail(),Toast.LENGTH_LONG).show();
